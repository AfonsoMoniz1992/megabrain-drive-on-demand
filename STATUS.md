...
| BRAT installation/update | Public v1.0.7 beta artefact; no broker or OAuth client is included | Install only in a disposable/test vault; copy the full canonical device fingerprint and configure an operator-owned broker URL and test-root name before enrolling a device.
